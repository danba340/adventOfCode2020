...
const validPassports = passports.split("\n\n").filter((passport) => {
  return requiredFields.every((requiredField) => {
    return passport.includes(requiredField);
  });
});

const validField = (field, passport) => {
  if (!passport.includes(field + ":")) return false;
  const startIndex = passport.indexOf(field + ":") + 4;
  const nextColonIndex =
    passport.substring(startIndex).indexOf(":") + startIndex;
  let endIndex = passport.substring(startIndex).indexOf(" ") + startIndex;
  if (endIndex <= startIndex || endIndex > nextColonIndex) {
    endIndex = passport.substring(startIndex).indexOf("\n") + startIndex;
  }
  if (endIndex <= startIndex || endIndex > nextColonIndex) {
    endIndex = passport.length;
  }
  const data = passport.substring(startIndex, endIndex);
  console.log(data);
  if (field === "byr") {
    return (
      data.length === 4 && parseInt(data) >= 1920 && parseInt(data) <= 2002
    );
  } else if (field === "iyr") {
    return (
    ...
    return (
      data.length === 4 && parseInt(data) >= 2020 && parseInt(data) <= 2030
    );
  } else if (field === "hgt") {
    if (data.substring(data.length - 2) === "in") {
      return (
        parseInt(data.substring(0, data.length - 2)) >= 59 &&
        parseInt(data.substring(0, data.length - 2)) <= 76
      );
    } else if (data.substring(data.length - 2) === "cm") {
      return (
        parseInt(data.substring(0, data.length - 2)) >= 150 &&
        parseInt(data.substring(0, data.length - 2)) <= 193
      );
    } else {
      return false;
    }
    ...
    console.log("Warning: no mathing field");
    return false;
  }
};
const validPassportsDataValidated = passports
  .split("\n\n")
  .filter((passport) => {
    return requiredFields.every((requiredField) => {
      const valid = validField(requiredField, passport);
      console.log(requiredField, valid);
      return valid;
    });
  });

console.log(validPassportsDataValidated.length);
