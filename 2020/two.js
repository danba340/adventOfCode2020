...
const validPolicyOneCount = passwords.filter((password) => {
  const dashIndex = password.indexOf("-");
  const min = parseInt(password.substring(0, dashIndex));
  const letterIndex = password.indexOf(":") - 1;
  const letter = password[letterIndex];
  const max = parseInt(password.substring(dashIndex + 1, letterIndex - 1));
  const occurences =
    password.substring(letterIndex + 1).split(letter).length - 1;
  return occurences >= min && occurences <= max;
}).length;

const validPolicyTwoCount = passwords.filter((password) => {
  const dashIndex = password.indexOf("-");
  const pos1 = parseInt(password.substring(0, dashIndex));
  const letterIndex = password.indexOf(":") - 1;
  const letter = password[letterIndex];
  const pos2 = parseInt(password.substring(dashIndex + 1, letterIndex - 1));
  const onlyPassword = password.substring(letterIndex + 3);
  return (
    (onlyPassword[pos1 - 1] === letter && onlyPassword[pos2 - 1] !== letter) ||
    (onlyPassword[pos1 - 1] !== letter && onlyPassword[pos2 - 1] === letter)
  );
}).length;

console.log(validPolicyTwoCount);
console.log("done");
