const rulesString = `muted lime bags contain 1 wavy lime bag, 1 vibrant green bag, 3 light yellow bags.
...
for (const rule of rules) {
  const [bag, containedBagsString] = rule.split(" bags contain ");
  const containedBags = containedBagsString
    .replace(".", "")
    .split(" bags")
    .join("")
    .split(" bag")
  ...
  canContainMap[bag] = containedBags;
}

const createBooleanSearchTree = (canContainMap, bagColor) => {
  const results = canContainMap[bagColor].map((innerBag) => {
    if (innerBag.color.includes("shiny gold")) {
      return true;
    } else if (!innerBag.color.includes("no other")) {
      return createBooleanSearchTree(canContainMap, innerBag.color);
    }
  });
  return results;
};

const recursivelyFindValue = (arr, value) => {
  if (arr) {
    for (var i = 0; i < arr.length; i++) {
      if (arr[i] === value) {
        return value;
      }
      const found = recursivelyFindValue(arr[i], value);
      if (found) return found;
    }
  }
};

const canContainShiny = Object.keys(canContainMap).filter((bagColor) => {
  const booleanSearchTree = createBooleanSearchTree(canContainMap, bagColor);
  const result = recursivelyFindValue(booleanSearchTree, true);
  return result;
});

const countQtyRecursively = (
  canContainMap,
  bagColor,
  count = 0,
  multiple = 1
) => {
  canContainMap[bagColor].forEach((innerBag) => {
    count += innerBag.qty * multiple;
    if (!innerBag.color.includes("no other")) {
      count = countQtyRecursively(
        canContainMap,
        innerBag.color,
        count,
        innerBag.qty * multiple
      );
    }
  });
  return count;
...
const qtyInsideShinyGold = countQtyRecursively(canContainMap, "shiny gold");
console.log(qtyInsideShinyGold);
