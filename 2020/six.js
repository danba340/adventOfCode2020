...
const groupsYesCount = groups.map(group => {
    const answersString = group.split("\n").join("")
    const uniqueYeses = new Set();
    for(const char of answersString){
        uniqueYeses.add(char)
    }
    return uniqueYeses.size
})

const sumOfGroupYeses = groupsYesCount.reduce((acc, groupYesCount) => {
    return acc + groupYesCount;
}, 0)


const groupsAllYesCount = groups.map(group => {
    const answers = group.split("\n").filter(answer => answer.length)
    const answersString = group.split("\n").join("");
    let allYesCount = 0
    for(const letter of aToZ){
        if((answersString.match(new RegExp(letter, "g")) || []).length >= answers.length) {
            allYesCount++
        }
    }
    return allYesCount
...
const sumOfGroupAllYeses = groupsAllYesCount.reduce((acc, groupAllYesCount) => {
    return acc + groupAllYesCount;
}, 0)

console.log(sumOfGroupAllYeses)